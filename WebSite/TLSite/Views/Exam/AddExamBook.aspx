		...
		<form class="form-horizontal" role="form" id="validation-form">
			...
			<div class="form-group" >
                <div class="col-sm-6">
				    ...
				    <div class="col-sm-6">
                        <div class="clearfix">
						    <select class="select2" id="examkind" name="examkind" data-placeholder="请选择..."  <% if (userrole != null && ((string)userrole).Contains("TeamManager")) { %> disabled<% } %> onchange="showTeamList();RefreshExamList();">
                    ...
                    <label class="col-sm-6 control-label no-padding-right teamlist <% if ((bookinfo==null&&!((string)userrole).Contains("TeamManager"))||(bookinfo!=null&&!((string)userrole).Contains("TeamManager")&&bookinfo.examkind != ExamKind.Team)||((string)userrole).Contains("TeamManager")) { %> hide <% } %>" for="examkind">车队<span class="red">*</span>：</label>
				    <div class="col-sm-6 teamlist <% if ((bookinfo==null&&!((string)userrole).Contains("TeamManager"))||(bookinfo!=null&&!((string)userrole).Contains("TeamManager")&&bookinfo.examkind != ExamKind.Team)||((string)userrole).Contains("TeamManager")) { %> hide <% } %>">
                        <div class="clearfix">
						    <select class="select2" id="teamid" name="teamid" data-placeholder="请选择..." onchange="RefreshExamList();">
                               <% var teamlist = (List<tbl_railteam>)ViewData["teamlist"];
                               foreach (var item in teamlist)
                               {
                                   %>
                                   <option value="<%= item.uid %>" <% if((bookinfo != null && bookinfo.teamid == item.uid)){ %> selected <%}%> > <%= item.teamname %></option>
                                   <%
                               }
                                %>
				            </select>
				...
				<label class="col-sm-1 control-label no-padding-right" for="">考试内容<span class="red">*</span>：</label>
				<div class="col-sm-3">
                    <div class="clearfix">
	...
	<script type="text/javascript">
	    ...
	    function redirectToListPage(status) {
	        if (status.indexOf("error") != -1) {
                ...
                <%if (((string)CommonModel.GetUserRoleInfo()).Contains("TeamManager")) {%>
	            window.location = rootUri + "Exam/TeamExamBookList";
                <% 
                }
                else{%>
	            window.location = rootUri + "Exam/ExamBookList";
	           <%  }%>
            }
	    }
	    jQuery(function ($) {
			...
			.dataTable({
			    ...
			    "bFilter": true,
			    "bLengthChange": false,
			    "sAjaxSource": rootUri + "Exam/RetrieveExamListWithCondition?examkind="+$("#examkind").val()+"&&teamid="+$("#teamid").val(),
			    "oLanguage": {
			        "sUrl": rootUri + "Content/i18n/dataTables.chinese.txt"
			    },
			    //bAutoWidth: false,
...
//                "fnDrawCallback": function () { $("#tblexamlib_wrapper").children("div.col-xs-6").append("<select> 试题种类</select>");},
        ...
        function changetesttype(){
              if(tblexamlib!=null){
                 ...
                 oSettings.sAjaxSource = rootUri + "Exam/SerchExamList?testtype="+$("#testtype").val()+"&examkind="+$("#examkind").val()+"&teamid="+$("#teamid").val();
        ...
        function randomgenerate(){
            ...
            if(tblexamlib!=null){
                 var oSettings = tblexamlib.fnSettings();                 
                 oSettings.sAjaxSource = rootUri + "Exam/GenerateExamDataTable?testtype="+$("#testtype").val()+"&testamount="+testamount+"&examkind="+$("#examkind").val()+"&teamid="+$("#teamid").val();
                 //oSettings.bPaginate= false;
                 oSettings._iDisplayLength= parseInt(testamount);
                 tblexamlib.fnClearTable(0);
                 tblexamlib.fnDraw();
              }
        }
        ...
        function RefreshExamList(){
            if (tblexamlib != null) {
                    var oSettings = tblexamlib.fnSettings();
                    oSettings.sAjaxSource = rootUri + "Exam/RetrieveExamListWithCondition?examkind="+$("#examkind").val()+"&&teamid="+$("#teamid").val();
                    tblexamlib.fnClearTable(0);
                    tblexamlib.fnDraw();
                }
        }
