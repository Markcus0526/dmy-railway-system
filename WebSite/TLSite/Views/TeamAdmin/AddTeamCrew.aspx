		...
		<form class="form-horizontal" role="form" id="validation-form">
		    ...
		    <div class="form-group" >
			    ...
			    <div class="col-sm-9" style="margin:6px 0px;">
                    <div class="clearfix">
				    <input type="text" id="crewno" name="crewno" placeholder="请输入工次号" class="input-large form-control" <% if (userinfo != null) { %>value="<%= userinfo.crewno %>"<% } %>  <% if (userinfo != null&&userinfo.crewno!=null) { %>readOnly<% } %>/>
                    </div>
			    </div>
		    </div>
		    <div class="form-group" >
          ...
          <%  if (userinfo == null)
			    ...
			    <div class="col-sm-9" style="margin:6px 0px;">
                    <div class="clearfix">
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="2" 
                                    class="ace"  />
							    <span class="lbl"> 车队干部</span>
						    </label>
					    </div>
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="1" class="ace" checked/>
							    <span class="lbl"> 车队人员</span>
						    </label>
					    </div>
                           ...
                           <%if (ViewData["teamlist"] != null)
                             {
                                 foreach (var n in (List<tbl_railteam>)ViewData["teamlist"])
                                      {
                                     %><option value="<%=n.uid %>"  <%if(userinfo != null && userinfo.teamid==n.uid){ %>selected <% }  %>><%=n.teamname%></option> <%
                                     }
                                 }%>
				        </select>
                    </div>
		    ...
		    <div class="form-group teamcrew" id="groupdiv" >
			    <label class="col-sm-3 control-label no-padding-right" style="margin:6px 0px;" for="groupid" >所属班组：</label>
			    <div class="col-sm-3" style="margin:6px 0px;">
                    <div class="clearfix">
					    <select class="select2" id="groupid" name="groupid" data-placeholder="请选择班组" <%if( userinfo != null &&gorupmodel.CheckIfDailiGroup(userinfo.crewgroupid)){%>readOnly<%} %>>
                            <%if (userinfo != null && gorupmodel.CheckIfDailiGroup(userinfo.crewgroupid))
                              {
                              %><option value="<%=userinfo.crewgroupid %>"><%=gorupmodel.GetGroupById(userinfo.crewgroupid).groupname%></option>     <%
                              }
                              else
                              {%>
                                <% if (ViewData["grouplist"] != null)
                                   {
                                       ...
                                       {
                                        %>
                                        <option value="<%=n.uid %>" <%if(userinfo != null && userinfo.crewgroupid==n.uid){ %>selected <% }  %> ><%=n.groupname%></option>
                                        <%
                                       }
                                   }
                              }%>
            ...
            <div class="form-group teamexec" style="display:none;" >
	...
	<script type="text/javascript">
	    function redirectToListPage(status) {
	        if (status.indexOf("error") != -1) {
	            ...
	            window.location = rootUri + "TeamAdmin/TeamCrewList";
	        }
	    }
	    jQuery(function ($) {
	        $('.loading-btn')
            ...
            $("#parentid").change(function(){
                ...
                $.ajax({
                    async: false,
                    type: "GET",
                    url: rootUri + "TeamAdmin/GetGroupListWithoutDailyGroup",
                    dataType: "json",
                    data: {
                        teamid: teamid
                    },
...
//            });
       ...
       // alert($('#validation-form').serialize());
	        $.ajax({
	            type: "POST",
	            url: rootUri + "TeamAdmin/SubmitTeamCrew",
	            dataType: "json",
