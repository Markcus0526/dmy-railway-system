...
<% var userinfo = (UserDetailInfo)ViewData["userinfo"]; %>
<% if (userinfo != null)
   {%>
    <input type="text" id="userid" value="<% =userinfo.uid %>" style="display:none"/>
<%} %>
...
<div class="row">
	<div class="col-xs-12">
		<form class="form-horizontal" role="form" id="validation-form">
		    ...
		    <div class="form-group" >
			    <label class="col-sm-3 control-label no-padding-right" style="margin:6px 0px;" for="exectype">人员类型：</label>
			    <div class="col-sm-9" style="margin:6px 0px;">
                    <div class="clearfix">
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="<%= ExecType.SectorExec %>" 
                                    class="ace"  <% if (userinfo == null || (userinfo != null && userinfo.usertype == ExecType.SectorExec)) { %>checked<% } %> />
							    <span class="lbl"> <%= ExecType.SectorExec %></span>
						    </label>
					    </div>
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="<%= ExecType.TeamExec %>" class="ace" <% if (userinfo != null && userinfo.usertype == ExecType.TeamExec) { %>checked<% } %>/>
							    <span class="lbl"> <%= ExecType.TeamExec %></span>
						    </label>
					    </div>
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="<%= ExecType.TrainCoach %>" class="ace" <% if (userinfo != null && userinfo.usertype == ExecType.TrainCoach) { %>checked<% } %>/>
							    <span class="lbl"> <%= ExecType.TrainCoach %></span>
						    </label>
					    </div>
                        <div class="radio" style="display:inline-block">
						    <label>
							    <input name="exectype" type="radio" value="<%= ExecType.TrainCrew %>" class="ace" <% if (userinfo != null && userinfo.usertype ==ExecType.TrainCrew ) { %>checked<% } %>/>
							    <span class="lbl"> <%= ExecType.TrainCrew%></span>
						    </label>
					    </div>
                    </div>
			    </div>
		    ...
		    <div class="form-group" >
                    ...
                    <div class="clearfix">
					    <select class="select2" id="parentid" name="parentid" data-placeholder="请选择部门">
                             <% if (ViewData["parentlist"] != null) {
                                    if (userinfo == null || (userinfo != null && userinfo.usertype == ExecType.SectorExec))
                                   {
                                    ...
                                    else if (userinfo != null && userinfo.usertype != ExecType.SectorExec)
                                   {
                                       foreach (var item in (List<tbl_railteam>)ViewData["parentlist"])
                                       { %>
                                        <option value="<%= item.uid %>"  <% if ((userinfo != null && item.uid == userinfo.execparentid)|| (userinfo != null && item.uid == userinfo.teamid)){ %>selected<% } %>><%= item.teamname %></option>
                                       <% }
                                   }
                                %>
                            <% 
                ...
                <label class="opinionman <% if (userinfo == null || (userinfo != null && userinfo.usertype != ExecType.TeamExec)) { %>hide<% } %> col-sm-2 control-label no-padding-right" style="margin:6px 0px;" for="opinionmanage">管理诉求：</label>
			    <div class="col-sm-4 opinionman <% if (userinfo == null || (userinfo != null && userinfo.usertype != ExecType.TeamExec)) { %>hide<% } %>" style="margin:6px 0px;">
                    <div class="clearfix">
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="opinionmanage" type="radio" value="1" 
                                    class="ace"  <% if (userinfo != null && userinfo.opinionmanage == 1) { %>checked<% } %> />
					    ...
					    <div class="radio" style="display:inline-block">
						    <label>
							    <input name="opinionmanage" type="radio" value="0" 
                                    class="ace" <% if (userinfo == null || (userinfo != null && userinfo.opinionmanage == 0)) { %>checked<% } %>/>
		    ...
		    <div class="form-group <% if (userinfo == null||(userinfo != null && userinfo.userkind !=2)) { %>hide<% } %>" id="groupdiv" >
			    <label class="col-sm-3 control-label no-padding-right" style="margin:6px 0px;" for="groupid">所属班组：</label>
			    <div class="col-sm-3" style="margin:6px 0px;">
                    <div class="clearfix">
		    ...
		    <div class="form-group exe <% if ((userinfo != null && userinfo.usertype == ExecType.TrainCoach)||(userinfo != null && userinfo.usertype == ExecType.TrainCrew)) { %>hide<% } %>">
			    <label class="col-sm-3 control-label no-padding-right " style="margin:6px 0px;" for="execrole">职务：</label>
			    <div class="col-sm-3" style="margin:6px 0px;">
                    <div class="clearfix">
				    <input type="text" id="execrole" name="execrole" placeholder="请输入职务" class="input-large form-control" <% if (userinfo != null) { %>value="<%= userinfo.execrole %>"<% } %> />
                    </div>
			    </div>

                <label class="opinionman <% if (userinfo == null || (userinfo != null && userinfo.usertype != ExecType.TeamExec)) { %>hide<% } %> col-sm-2 control-label no-padding-right" style="margin:6px 0px;" for="teammanage">
                车队管理：</label>
			    <div class="col-sm-4 opinionman <% if (userinfo == null || (userinfo != null && userinfo.usertype != ExecType.TeamExec)) { %>hide<% } %>" style="margin:6px 0px;">
                    <div class="clearfix">
					    <div class="radio" style="display:inline-block">
		    ...
		    <div class="form-group crew <% if (userinfo==null||(userinfo != null && userinfo.usertype != ExecType.TrainCrew)) { %>hide<% } %>" >
			    <label class="col-sm-3 control-label no-padding-right" style="margin:6px 0px" for="crewrole" >职务：</label>
			    <div class="col-sm-9" style="margin:6px 0px;">
                    <div class="clearfix">
                        <select class="select2 " id="crewrole" name="crewrole" data-placeholder="请选择职务" >
                            <% if (ViewData["crewrole"] != null){
                                //   var groupinfo = (tbl_traingroup)ViewData[""];
                                    foreach (var item in (List<tbl_crewrole>)ViewData["crewrole"])
                                    { %>
                                    <option value="<%= item.uid %>" <% if (userinfo != null && item.uid == userinfo.crewroleid) { %>selected<% } %>><%= item.rolename %></option>
                                    <% }                                       
                                %>
                            <% 
                                }  %>
	        ...
	        $('input[name="exectype"]:radio').change(
                function ()  {
                    ...
                    $.ajax({
                        async: false,
                        type: "GET",
                        url: rootUri + "User/GetExecutiveParentList?uid="+uid,
                        dataType: "json",
                        data: {
                            exectype: exectype
                        },
                        success: function (data) {
                            ...
                            if (exectype == "科室干部" || exectype == "车队干部") 
                            {
                                if (data.length > 0) {
                                    ...
                                    $("#parentid").html(rhtml);
                                    $("#parentid").css('width', '200px').select2({ allowClear: true });
                                ...
                                $("#groupdiv").removeClass("hide");
                                $(".opinionman").addClass("hide");
                                $(".exe").addClass("hide");
                                $(".crew").addClass("hide");
                            }else if (exectype == "列车员") {
                            for (var i = 0; i < data.teamlist.length; i++) {
                                    rhtml += "<option value='" + data.teamlist[i].uid + "'>" + data.teamlist[i].teamname + "</option>";
                                }
                                $("#parentid").html(rhtml);
                                ...
                                for (var i = 0; i < data.grouplist.length; i++) {
                                    rhtml += "<option value='" + data.grouplist[i].uid + "' "+ test(i,data) + "  >" + data.grouplist[i].groupname + "</option>";
                                }
                                $("#groupid").html(rhtml);
                                $("#groupid").css('width', '200px').select2({ allowClear: true });
...
//         var exectype;
//         if (userinfo != null) {
//             exectype=userinfo.exectype;
//         }
//         else{
//             exectype=$('input[name="exectype"]:radio').val();
//         }

//            if (exectype == "科室干部" || exectype == "车队干部"||exectype == "列车长")  {
//                $(".exe").show();
//                $(".crew").hide();
//            }
//            else{
...
//            });
        ...
        if (data.selectinfo!= null) {
            if (data.selectinfo.groupid==data.grouplist[i].uid){
                return "selected";
            }
        }
        }
