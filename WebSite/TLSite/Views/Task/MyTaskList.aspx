		...
		<script type="text/javascript">
		    ...
		    jQuery(function ($) {
				...
				.dataTable({
				    "bServerSide": true,
				    "bProcessing": true,
				    "sAjaxSource": rootUri + "Task/RetrieveMyTaskList?kind=<%= ViewData["kind"] %>",
				    "oLanguage": {
				    ...
				    "aoColumnDefs": [
				        ...
				        },
				        {
				            ...
				            fnRender: function (o, v) {   // o, v contains the object and value for the column
				                var rst = "";
				                if (o.aData[8].match("待接收")) {
				                    rst += '<a class="btn btn-xs btn-info" onclick="processExecute(' + o.aData[10] + ')">' +
                                    '<i class="ace-icon fa fa-pencil bigger-120"></i>接收' +
                                    '</a>&nbsp;&nbsp;';
				                } else if (o.aData[8].match("执行中")) {
				                    rst += '<a class="btn btn-xs btn-info" onclick="processFinish(' + o.aData[10] + ')">' +
                                    '<i class="ace-icon fa fa-pencil bigger-120"></i>完成' +
                                    '</a>&nbsp;&nbsp;';
				                }

				                rst += '<a class="btn btn-xs btn-info" href="' + rootUri + 'Task/TaskDetail/' + o.aData[10] + '?origin=mytask">' +
		    ...
		    function processFinish(taskid) {
		        if (confirm("您确定要完成该任务吗?")) {
		            $.ajax({
		                url: rootUri + "Task/ExecuteFinish/" + taskid,
		                type: "post",
		                success: function (message) {
		                    if (message == "") {
		                        toastr.options = {
            ...
            function processExecute(taskid) {
                if (confirm("您确定要接收该任务吗?")) {
                    $.ajax({
                        url: rootUri + "Task/ExecuteTask/" + taskid,
                        type: "post",
                        success: function (message) {
                            if (message == "") {
                                toastr.options = {
